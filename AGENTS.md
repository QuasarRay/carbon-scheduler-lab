...
## Carbon Scheduler project notes

- Treat `README.md`, `CMakeLists.txt`, `CMakePresets.json`, `include/`, `src/`, `python/`, and `tests/` as the current project surface unless a task establishes a newer source of truth.
- Scheduler and channel behavior is compatibility-sensitive. Preserve Tasklet/channel scheduling order and Stackless-Python-compatible behavior unless the task explicitly changes that contract.
- When changing Python or C-API exposure, keep docstrings and C++ documentation blocks aligned with behavior.
- Prefer the smallest behavior-preserving change and run the closest relevant build/tests. Report platform or dependency gaps instead of claiming checks that were not run.
